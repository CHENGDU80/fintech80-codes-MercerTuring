...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 10 == 0:
        print(
            f'Epoch [{epoch + 1}/{num_epochs}], Loss: {loss.item():.4f}, Val Loss: {val_loss.item():.4f}, Val Accuracy: {val_accuracy:.4f}')
...
# 进行推测
predictions = predict(new_samples)
print("推测的风险等级:", predictions)
torch.save(model.state_dict(), 'HumanRiskmodel_weights.pth')